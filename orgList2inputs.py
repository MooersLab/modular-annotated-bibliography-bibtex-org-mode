...
"""
Purpose: read a list of citekeys as strings from the file "papersAboutCoding.org", 
reformat for use in main.tex, and write them to "citations.org".
...
This script includes error handling to check if the file papersAboutCoding.org exists. 
If the file is not found, it prints an error message and proceeds with an empty list of strings.

The second function, create_bib_notes_file(), creates a new tex file in the ./bibNotes (or ~/bibNotes) directory, 
for each citekey, if the file does not already exist. 
We write out tex because org-mode can read it with the include command.

This script takes the input and output filenames as command line arguments. 
Run it from the command line like this:

python script.py papersAboutCoding.org citations.org
...
def format_citations(strings):
    """Assume that the citekeys (the strings) are in an itemized list in org format lead by `- `. 
...
def create_bib_notes_file(string):
    ...
    This folder could be a subfolder or a global folder. 
    If the latter, replace the '.' with a '~'. 
    This function will create a bibNotes folder if it does not exist.
    It will not overwrite existing bibNotes with the same citekey."""
    
    directory = "./bibNotes"
    if not os.path.exists(directory):
        os.makedirs(directory)
    
    file_path = os.path.join(directory, f"{string}.tex")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("The input filename is a list citekeys as Org - items ")
